...
class Notification_queues
{
	...
	public function __construct()
	{
		...
		$this->CI->load->library('Php_serial');
	}
	...
	public function insert_email_queues( $message, $from_user, $to_user, array $email_data = array() )
	{
		...
		try
		{
			...
			{
                $default_data['email_data']['cc_email'] = PRIA_ALERTS;
	...
	private function _send_sms( array $sms_to_send )
	{
		try
		{
			...
			$flag 	= $this->CI->php_serial->sendMessageToUser($sms_to_send['mobile_no'], $sms_to_send['message']);
