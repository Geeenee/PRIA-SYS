...
class Upload_soa extends Task_Controller 
{
    ...
    public function index()
    {
        try
        {
            ...
            $where          = ['document_type_code' => DOC_TYPE_SOA, 'reference' => $this->task_details['reference_id']];
            ...
            $this->task_view_data['soa_details'] = $soa_details;
    ...
    public function process()
    {
        try
        {
            ...
            if($task_status_id == TASK_STATUS_DONE AND EMPTY($soa_details['submission_date']))
            {
                $sub_val = array('submission_date' => date(FORMAT_DB_DATETIME));
                $this->soa_model->update_soa($where, $sub_val);
            }
...
*/
            ...
            $this->tag_task($task_id, $task_status_id, $val, NULL, $soa_details['recipient_id']);
