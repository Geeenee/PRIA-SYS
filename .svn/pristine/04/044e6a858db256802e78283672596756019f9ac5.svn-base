...
class Medvac extends Task_Controller 
{
		...
		{  
         ...
         /*    $sub_nav_left_config	 = [
            ...
            $this->task_view_data['dependent_task'] = NULL;
             ...
             if( ! EMPTY($dgr_id))
             {
                $fields  = [ 'dr_num', 'dr_date', 'transaction_date', 'last_dr_flag', 'dr_gr_id' ];
                $this->task_view_data['medvac_details'] = $this->dgr_model->get_delivery_goods_receipt(['dr_gr_id' => $dgr_id], $fields);
            ...
            $this->task_page                  = '/encode_medvac_dr';
    ...
    public function process()
    {
        try
        {
            ...
            $fields       = [
                'dr_num'             => $data['dr_transaction_no'],
                'dr_date'            => std_db_date_format($data['medvac_dr_date']),
                'transaction_date'   => std_db_date_format($data['transaction_date']),  
                'last_dr_flag'       => ISSET($data['medvac_deliveries']) ? $data['medvac_deliveries']: ENUM_NO,  
            ];
            ...
            {   
                ...
                $prev_detail  = [array()];
                $activity     = sprintf($this->lang->line('audit_trail_add'), ' MedVac');
               ...
               /*   $this->pria_workflow->tag_task_status($task_id, $task_status_id, [
                    'reference'         => $dgr_id, 
                    'start_date'        => date(FORMAT_DB_DATETIME),
                    'actual_start_date' => std_db_date_format($data['medvac_dr_date']),
                    'actual_end_date'   => std_db_date_format($data['transaction_date']) 
                ]); */
            ...
            {   
                ...
                $activity     = sprintf($this->lang->line('audit_trail_update'), ' MedVac');
    ...
    private function _validate()
    {
        try
        {
            ...
            ->filter_string('dr_transaction_no')
            ->filter_date('medvac_dr_date')
            ->filter_date('transaction_date')
            ->filter_string('medvac_deliveries')
            ->filter();
            ...
            $required = [
                'dr_transaction_no' => 'DR Transaction No.',
                'medvac_dr_date'    => 'DR Date',
                'transaction_date'  => 'Transaction Date'
            ];

            $constraints['dr_transaction_no']	= [
                'data_type'			=> 'string',
                'name'				=> 'DR Transaction No.'
            ];
    
            $constraints['medvac_dr_date']	    = [
                'data_type'			=> 'date',
                'name'				=> 'DR Date'
            ];
            
            $constraints['transaction_date']	= [
                'data_type'			=> 'date',
            ...
            if( ISSET($params['medvac_deliveries']) )
            {   
                $constraints['medvac_deliveries']	= array(
                    'data_type'			=> 'enum',
                    'name'				=> 'Last MedVac Delivery',
            ...
            $start_date = strtotime($data['medvac_dr_date']);
            $end_date   = strtotime($data['transaction_date']);
            $curr_date  = strtotime(date(FORMAT_DB_DATE));

            if($start_date > $end_date)
                throw new Exception('MedVac DR date must not be greater than transaction date.');

            if($start_date > $curr_date)
                throw new Exception('MedVac DR date must not be greater than current date.');
                
            if($end_date > $curr_date)
                throw new Exception('Transaction date must not be greater than current date.');

            if (preg_match('#[0-9]#',$data['dr_transaction_no'])){

            }else{
                throw new Exception('DR Transaction number must have a numeric character.');
            }
