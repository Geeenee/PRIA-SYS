<script type="text/javascript">
	$(function(){
		...
		<?php if(!EMPTY($resources["load_js"])){ ?>
			...
			<!-- JPLIST PAGINATION/FILTERING -->
			<?php 
				if(ISSET($resources["load_js"]) AND in_array(JS_JPLIST_CORE, $resources["load_js"])){
					if(in_array(JS_JPLIST_PAGINATION, $resources["load_js"])){
						if( ISSET( $resources['paginate'] )){ 
							foreach($paginate as $id => $data):
			?>
							var id = "#<?php echo $id ?>",
								wrapper = ".list",
								item = ".list-item",
							...
							<?php if(!EMPTY($data)){ ?>
								<?php if(ISSET($data['wrapper'])){ ?>
									var wrapper = ".<?php echo $data['wrapper'] ?>";
								<?php } ?>
								
								<?php if(ISSET($data['item'])){ ?>
									var item = ".<?php echo $data['item'] ?>",
								<?php } ?>
								
								<?php if(ISSET($data['panel'])){ ?>
									var panel = ".<?php echo $data['panel'] ?>";
								<?php } ?>
							<?php } ?>
