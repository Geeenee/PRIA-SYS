...
class Upload_harvest extends Task_Controller 
{
    ...
    public function index()
    {
        try
        {
            ...
            {
                $fields  = ['harvest_rep_submit_date'];
            ...
            $this->task_page                = '/upload_harvest_report';
    ...
    public function process()
    {
        try
        {
            ...
            $where          = ['document_type_code' => DOC_TYPE_HARVEST, 'reference' => $task_details['reference_id']];
            $harvest_form   = $this->document_model->get_document($where);
            ...
            $fields       = [
                ...
                'harvest_rep_submit_by'     => $this->session->user_id
            ];
            ...
            if(EMPTY($io['harvest_rep_submit_date']))
            {
                ...
                if(EMPTY($harvest_form)){
                    if(EMPTY($data['doc_harvest'])){
                        throw new Exception('Harvest Report File is required.');
                    }
                }
                ...
                $prev_detail  = [array()];
                $activity     = sprintf($this->lang->line('audit_trail_add'), ' Harvest report');
    ...
    private function _validate()
    {
        try
        {
            ...
            $required = [
                ...
                // 'doc_harvest'    => 'Harvested Report File'
            ];
            ...
            $constraints['doc_harvest'] = [
                'data_type'         => 'string',
                'name'              => 'Harvested Report File'
            ];
            ...
            if($date_submitted > $curr_date)
                throw new Exception('Harvest report date must not be greater than current date.');
