...
class Ret_app_clean_up extends Task_Controller 
{
	...
	public function index()
	{
		try
		{
            ...
            if( ! EMPTY($io['actual_clean_up_date']))
            {
            ...
            $this->task_page                = '/ret_app_clean_up_report';
    ...
    public function return_task()
    {
        try{
            ...
            $return             = $this->pria_workflow->tag_task_status($task_id, TASK_STATUS_RETURNED);

            $flag               = SUCCESS;
            $msg                = $this->lang->line('succ_tagging_task_returned');
            $response['actor']  = $return['actor_name'];
