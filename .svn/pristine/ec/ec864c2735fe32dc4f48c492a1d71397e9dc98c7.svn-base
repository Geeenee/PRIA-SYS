...
class Indicate_store_opening extends Task_Controller 
{
	...
	public function index()
	{
		...
		{  
            ...
            $proj_details   = $this->pj_model->get_project(['project_id' => $proj_id], ['boq_id', 'opening_date', 'project_code', 'turnover_date']);
         ...
         /*    $sub_nav_left_config	 = [
            ...
            $this->task_page                  = '/indicate_store_opening';
    ...
    public function process()
    {
        try
        {
            ...
            if(strtotime($proj_details['turnover_date']) > strtotime($params['opening_date']))
            {
                throw new Exception('Opening date must not be earlier than the turnover date.');
            }
            ...
            $activity     = sprintf($this->lang->line($actvy_index), ' Indicate Store Opening Date');

           $fields        = [
                'opening_date'      => std_db_date_format($params['opening_date']),
 ...
 */
    ...
    private function _validate()
    {
        ...
        $constraints['task_status'] = [
         ...
         if($opening_date < $curr_date)
             throw new Exception('Opening date must not be less than current date.');
