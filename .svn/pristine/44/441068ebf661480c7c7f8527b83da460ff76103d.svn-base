...
class Upload_store_mockup_design extends Task_Controller 
{
    ...
    public function index()
    {
        try
        {
            ...
            $this->task_page                  = '/upload_store_mockup_design';
    ...
    public function process()
    {
        try
        {
            ...
            $activity     = sprintf($this->lang->line($actvy_index), ' Upload Store Mock-Up Design');
            ...
            $this->tag_task($task_id, $params['task_status'], ['reference' => $boq_id], NULL, $curr_detail[0][0]['created_by']);
    ...
    private function _validate($params, $task_details)
    {
        ...
        ->filter_string('doc_mockup_design')
        ...
        $task_id       = decrypt_id($params['etd']);
        $task_document = $this->dm_model->get_document(['pria_task_id' => $task_id, 'document_type_code' => DOC_TYPE_MOCKUP_DESIGN]);
         
        if(EMPTY($task_document))
        {
            $required['doc_mockup_design']    = 'Store Mock-Up Design';

            $constraints['doc_mockup_design'] = [
                'data_type'                 => 'string',
                'name'                      => 'Store Mock-Up Design'
            ];
        }
