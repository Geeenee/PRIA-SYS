...
?>
		...
		var <?php echo $id ?>_options_upload_obj 	= {
		    ...
		    {       
				...
				console.log(status);
				console.log(errMsg);
		    	//notification_msg('error', 'Invalid File aaa  bbb');
		    	end_loading();
				$("body").isLoading("hide");
			},
			customProgressBar: function()
			{
				this.statusbar = $('<div class="ajax-file-upload-statusbar">asdfasdf</div>');
			...
			{ 
				...
				/*
						@edited 	: kebs. 
						@date 		: Nov 8, 2019
						@purpose	: to move original file name at the top 

						Original code below.

						ajax_status_bar.first().find('.ajax-file-upload-red').last().after('<div>&nbsp;</div><div>Original Filename: <b>'+files+'</b></div>');
				*/
						ajax_status_bar.first().find('.ajax-file-upload-progress').last().before('<div>&nbsp;</div><div style="margin: 0px 0px 10px 0px;display: block;">Original Filename: <b>'+files+'</b></div>');
				<?php 
					endif;
				?>
 			...
 			/*	console.log($("#<?php //echo $upload['id'] ?>_orig_filename").parents('form').find('input:not(.added-orig)[type="hidden"][id="'+upload_id+'_orig_filename"]').first());
 				...
 				{
 					$("#<?php echo $id ?>_orig_filename").addClass( data[0] );
				...
				?>
					...
					$d.html("<a class='tooltipped' href='"+$base_url+"Upload/download?file="+data[0]+"&path="+dy_path+"' target='_blank' style='color:gray !important;' data-tooltip='View' data-position='bottom' data-delay='50' ><i class='material-icons'>visibility</i></a>");
						...
						success: function(data) 
						{
							...
							?>
								if( $("#<?php echo $id ?>").parents("form#<?php echo $id ?>_upload_form, div#<?php echo $id ?>_upload_form").length !== 0 )
								{
									...
									?>
									var $download 	= $("#<?php echo $id ?>").parents("form#<?php echo $id ?>_upload_form, div#<?php echo $id ?>_upload_form").find("div.ajax-file-upload-statusbar .ajax-file-upload-green:visible");
									<?php 
										endif;
									?>
								...
								else
								{
									...
									?>
									var $download 	= $("#<?php echo $id ?>").parents("form").find("div.ajax-file-upload-statusbar .ajax-file-upload-green:visible");
									<?php 
										endif;
									?>
								...
								for( ; i_inc < data_len; i_inc++ )
								{
									...
									$d.html("<a class='tooltipped' href='"+$base_url+"Upload/download?file="+$file+"&path="+dy_path+"' target='_blank' style='color:gray !important;' data-tooltip='View' data-position='bottom' data-delay='50' ><i class='material-icons'>visibility</i></a>");
		...
		//Added by kebs
		<?php if( ISSET( $upload['max_file_size'] ) AND !EMPTY( $upload['max_file_size'] ) ) :?>
			<?php echo $id ?>_options_upload_obj.maxFileSize 		= '<?php echo $upload['max_file_size'] ?>';
		<?php endif; ?>	
		...
		?>
			...
			autoSubObj.push('<?php echo $id ?>_uploadObj'); //added by kebs
		<?php 
			else :
		?>
