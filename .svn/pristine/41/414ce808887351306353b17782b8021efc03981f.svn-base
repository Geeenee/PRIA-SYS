...
class Renewal extends Transaction_Controller 
{
	public function __construct()
	{
		...
		$this->load->model(FOLDER_RENEWAL.'/renewal_model');
	}
		...
		try
		{
			...
			if($list['list']['next_records'] > 0)
			{
				$footer					= array(
						...
						'path'			=>  base_url().PORTAL_TRANSACTIONS.'/tabs/'.PORTAL_TAB_RENEWAL.'/page/'.$encoded_module_code.'/'.$encoded_tab_module.'/0/',
						'append'		=> 'li',
			...
			{
				$buttons[]				= array(
						'target'		=> 'modal_add_renewal',
						'label'			=> 'Add Recommendation',
						'class'			=> 'purple darken-1',
						'id'			=> 'add_renewal',
						'onclick'		=> 'modal_add_renewal_init(\''.$tab_module.'\',\'Add Recommendation\')'
				);
			}
	...
	public function page($encoded_module_code, $encoded_tab_module, $initial=TRUE, $page_num=1)
	{
		try
		{
			...
			$renewals					= $this->renewal_model->get_contract_renewal_list($where, NULL, $scope_details['having']);		
			
			//Get the next batch of internal orders
			$where['limit']['from']		= $page_num * SYS_SETTING_DISPLAY_LIST_NO;
			$next_records	 			= $this->renewal_model->get_contract_renewal_list($where, TRUE, $scope_details['having']);
			...
			if(in_array(AG_LESSORS, $ag_codes))
			{
				if(check_permission($tab_module, ACTION_EDIT))
				{
					$addtl_actions['edit']		= [
							...
							'onclick'		=> 'modal_add_renewal_init(\''.$tab_module.'\\\%s\',\'Edit Renewal\')'
					];
				}
			}
