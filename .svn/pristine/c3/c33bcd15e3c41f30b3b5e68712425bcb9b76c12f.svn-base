...
class Base_Controller extends MX_Controller
{
	...
	protected function pdf($filename, $html, $portrait = TRUE, $output = SYSAD_Controller::PDF_OUTPUT_STRING, $header = NULL, $footer = NULL, $margin_left = 10, $margin_right = 10, $margin_top = 10, $margin_bottom = 10, $margin_header=10, $margin_footer=10)
	{
		try
		{
			$this->core_reports->pdf($filename, $html, $portrait, $output, $header, $footer, $margin_left, $margin_right, $margin_top, $margin_bottom, $margin_header, $margin_footer );
		}
		catch(Exception $e)
		{
			throw $e;
	...
	protected function set_filter( array $values )
	{
	...
	protected function get_hash_key($name)
	{
		try
		{
			return "md5(CONCAT('" . self::HASH_CODE . "', $name, '" . self::HASH_CODE . "'))";
		}
		catch(Exception $e)
		{
			throw $e;
	...
	protected function hash($code)
	{
		try
		{
			return md5(self::HASH_CODE . $code . self::HASH_CODE);
		}	
		catch(Exception $e)
		{
			throw $e;
