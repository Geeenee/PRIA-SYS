		...
		try
		{
			...
			if(!EMPTY($result))
			{
				...
				foreach( $result['aaData'] as $r )
				{
					...
					//Starts
					if ($r['reminder_flag'] == YES_FLAG) continue;
					//Ends
				...
				/*	if( EMPTY( $r['read_date'] ) )
					...
					$link		= (ISSET($r['notification_html']) AND !EMPTY($r['notification_html']))? "<a href='".$r['notification_html']."'>%s</a>": "%s";

					$msg   		= sprintf($link, $r['notification']);
					...
					$html 		.= $msg . ' by ' . $r['notified_by_name'];
