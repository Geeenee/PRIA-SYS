...
$(function(){
	$('.nav-toggle').on('click', function() {
		var elem 		= $(this),
			...
			parentElem 	= $("#" + parentID),
			currWidth	= $('#' + parentID + '.sub-navigation').data('width');
			
		var collapseArrow	= parentElem.hasClass('sb-right') ? '&llarr;' : '&rrarr;';
			expandArrow		= parentElem.hasClass('sb-right') ? '&rrarr;' : '&llarr;';
		
		if(elem.hasClass('collapse'))
		{
			$('#' + parentID + '.sub-navigation').css('width','40px');
			
			elem.html(collapseArrow);
		}else{
			$('#' + parentID + '.sub-navigation').css('width',currWidth);
			elem.html(expandArrow);
		}
