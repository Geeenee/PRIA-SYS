var Soa = function() {
	...
	var save = function(e)
	{	
		console.log($base_url + $transactions + "/" + $module + "/soa/process");
		$("#submit_modal_add_soa").off('click').on("click", function(e){
			e.preventDefault();

			if($('#form_modal_add_soa').parsley().validate())
			{
				var data	= $("#form_modal_add_soa").serialize();

				button_loader('submit_modal_add_soa', 1);

				$.post($base_url + $transactions + "/" + $module + "/soa/process", data, function(result){
					notification_msg(result.status, result.msg);

					if(result.flag == '1')
					{
						$("#modal_add_soa").modal("close");

						$('a[href="#tab_soa"]').trigger('click');
					}
					
					button_loader('submit_modal_add_soa', 0);
		...
		if($('#business_center').length > 0)
		{
			...
			{
				const data	  = $("#form_modal_add_soa").serialize();
				...
				const options = {
					blockUI    : true,
					body 	   : data,
					path       : $base_url + $transactions + "/" + $module + "/soa/get_vendors",
				...
				$('#soa_document_recipient')[0].selectize.clear();
				$('#soa_document_recipient')[0].selectize.clearOptions();
				
				var options1 = {
					blockUI    : true,
					body 	   : $.param({'org_code' : val}),
					path       : $base_url + $transactions + "/" + $module + "/soa/get_recipients",
					successFunc: function(response){
						var len 		= response.length;
						var $select 	= $('#soa_document_recipient').selectize();
						var selectize 	= $select[0].selectize;
