...
class Notifications_model extends SYSAD_Model {
	...
	public function insert_notification($params, $notified_by=NULL )
	{
		try
		{
			$val = array(
				"notification" 			=> $params["notification"],
				"notified_by" 			=> ( ! EMPTY($notified_by)) ? $notified_by : $this->session->user_id,
			...
			//ADDITIONAL
			if(ISSET($params["notification_icon"]))
			{
				$val["notification_icon"] 	= $params['notification_icon'];
			}

			if(ISSET($params["module_code"]))
			{
				$val["module_code"] 	= $params['module_code'];
			}

			if(ISSET($params["notification_html"]))
			{
				$val["notification_html"] 	= $params['notification_html'];
			}

			if(ISSET($params["notification_mobile"]))
			{
				$val["notification_mobile"] 	= $params['notification_mobile'];
			}

			if(ISSET($params["displayed_socket_flag"]))
			{
				$val["displayed_socket_flag"] 	= $params['displayed_socket_flag'];
			}

			if(ISSET($params["listed_flag"]))
			{
				$val["listed_flag"] 	= $params['listed_flag'];
			}

			if(ISSET($params["reminder_flag"]))
			{
				$val["reminder_flag"] 	= $params['reminder_flag'];
			}
	...
	public function get_notifications($user_id, $user_roles, $user_orgs, $page = 0, $limit = 10, array $params = array(), array $filter = array(), array $ordering = array() )
	{
		...
		try
		{
			...
			$query 		= "
				...
				SELECT	SQL_CALC_FOUND_ROWS a.*, CONCAT_WS(' ', AGDEC(b.fname), AGDEC(b.lname)) notified_by_name
				FROM 	%s a
				JOIN %s b on a.notified_by = b.user_id
				WHERE 	a.reminder_flag = ? AND ( 
					$add_where
				)
				$filter_str
				$order
...
";
			...
			$query 		= sprintf( $query, SYSAD_Model::CORE_TABLE_NOTIFICATIONS, SYSAD_Model::CORE_TABLE_USERS );
