...
class Roles extends SYSAD_Controller 
{
	...
	public function get_role_list()
	{
		...
		try
		{
			...
			foreach ($roles as $arow)
			{
				...
				$delete_tooltip = ($arow['built_in'] == "Yes") ? "You are not allowed to delete built-in roles." : "Delete";
				$edit_tooltip = ($arow['built_in'] == "Yes") ? "You are not allowed to edit built-in roles." : "Edit";
	...
	{		
		try
		{
			...
			$resources['load_css'] 		= array(CSS_SELECTIZE, CSS_SUMO_SELECT);
			$resources['load_js'] 		= array(JS_SELECTIZE, JS_SUMO_SELECT);
			$resources['loaded_init'] 	= array(
				'Roles.initModal();',
				'Roles.save();'
			);
			$resources['selectize'] 	= array (
				'selectize-roles' 		=> array(
					'type' 				=> 'default'
				)
			);
