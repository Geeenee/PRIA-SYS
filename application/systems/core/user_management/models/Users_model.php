...
class Users_model extends SYSAD_Model 
{
	...
	public function get_user_list($filter_status = NULL, $params = NULL)
	{
		try 
		{
			...
			else
			{
				...
				/* 
					added distinct to the column below to make its data unique 
					as the addition of the newly added left join affects its function
					`
						GROUP_CONCAT(DISTINCT E.role_name SEPARATOR ",<br>") roles,
					`

					added code below to include the column in the select query
					`
						CASE 
							WHEN LENGTH(GROUP_CONCAT(DISTINCT G.name SEPARATOR ",<br>")) > 40 
							THEN CONCAT(SUBSTRING(GROUP_CONCAT(DISTINCT G.name SEPARATOR ",<br>"), 1, 40), "...") 
							ELSE GROUP_CONCAT(DISTINCT G.name SEPARATOR ",<br>") 
						END organizations
					`
			...
			// Change Request 02.15.23 Starts Here
			/* 
				added the join query below to get the organizations of the user per row
				`
					LEFT JOIN ' . parent::PORTAL_TABLE_USER_ORGS . ' F ON F.user_id = A.user_id
					LEFT JOIN ' . parent::CORE_TABLE_ORGANIZATIONS . ' G ON G.org_code = F.org_code 
				`
			*/
	...
	private function _construct_filters($signup,  &$params)
	{
		...
		else 
		{
			...
			// Change Request 02.15.23 Starts Here
			// Added the line below to include the advance filter of the user organization in query
			if( isset($params['organizations']) )
				$params['G.name']	= $params['organizations'];
			
			$addtl_filter_col = 'G.name';
