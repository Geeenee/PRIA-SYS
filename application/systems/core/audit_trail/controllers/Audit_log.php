...
class Audit_log extends SYSAD_Controller 
{
	...
	{	
		try
		{
			...
			$resources['load_css'] 	= array(CSS_DATETIMEPICKER, CSS_DATATABLE_MATERIAL, CSS_SELECTIZE);
			$resources['load_js'] 	= array(JS_DATETIMEPICKER, JS_DATATABLE, JS_DATATABLE_MATERIAL, $module_js);
	...
	public function print_form()
	{
		...
		try
		{
			...
			$aColumns 	= array("A.*", "B.photo", "DATE_FORMAT(activity_date,'%m/%d/%Y %T') activity_date", "IF(A.user_id = 0, AGDEC(B.fname) ,CONCAT(AGDEC(B.fname),' ', AGDEC(B.lname))) name", "C.module_name","B.fname" );
