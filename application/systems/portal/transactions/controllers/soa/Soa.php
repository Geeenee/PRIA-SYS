...
class Soa extends Task_Controller 
{
    ...
    public function modal_add_soa($tab_module = NULL, $security = NULL)
    {
        try 
        {
            ...
            $soa_id                 = (!EMPTY($security))? decrypt_id($security): NULL;
 ...
 */
    ...
    public function process()
    {
        try
        {
            ...
            $tab_module_details   = $this->soa_model->get_tab_module($where);

            if(EMPTY($data['security']))
            {
                //check if soa number already exist
                $where      = array( 'soa_num' => $data['soa_num'], 'vendor_code' => $data['vendor']);
                $has_soa    = $this->soa_model->get_soas($where);

                if($has_soa){
                    throw new Exception('SOA number already exists.');
                }

                if( !EMPTY($data['soa_num'])){
                    ...
                    $soa_id = $this->soa_model->insert_soa($insert);
                    ...
                    $this->soa_model->update_workflow($workflow_fields, $workflow_where);

                    $this->tag_task($workflow_det['task_id'], TASK_STATUS_ONGOING, [
                        'reference' => $soa_id, 'manual_get' => ENUM_YES
                    ]);
    ...
    private function _validate()
    {
        try
        {
            ...
            $constraints['soa_date_submitted']    = [
                'data_type'         => 'date',
                'name'              => 'SOA Submitted Date'
            ];
            ...
            $constraints['soa_amount']    = [
                'data_type'         => 'amount',
                'name'              => 'SOA amount',
                'max'               => 99999999.99
            ];

            $constraints['soa_document_recipient']    = [
                'data_type'         => 'string',
    ...
    public function get_vendors()
    {
        try
        {
            ...
            {
                //Changed by Christian Oct 22, 2019
                //$scope_details  = get_scope_details($this->main_module);
                $scope_details  = get_scope_details($tab_module);

                //$options        = $this->soa_model->get_vendor_by_org_code_arr($params['business_center'], $scope_details['vendor_code'], ['a.vendor_code as value', 'a.vendor_name as text']);

                $options        = $this->soa_model->get_vendor_by_org_code_arr_and_ag_arr($ag_codes, $params['business_center'], $scope_details['vendor_code'], ['a.vendor_code as value', 'CONCAT("[", a.vendor_code, "] ", a.vendor_name) as text']);
