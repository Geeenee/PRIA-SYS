...
class Purchase_requests extends Transaction_Controller 
{
	public function __construct()
	{
		...
		$this->load->model(FOLDER_PURCHASE_REQUESTS.'/purchase_requests_model', 'pr_model');
	}
		...
		try
		{
			...
			if($list['list']['next_records'] > 0)
			{
				$footer					= array(
						...
						'path'			=>  base_url().PORTAL_TRANSACTIONS.'/tabs/'.PORTAL_TAB_PURCHASE_REQUESTS.'/page/'.$encoded_module_code.'/'.$encoded_tab_module.'/0/',
						'append'		=> 'li',
			...
			if(check_permission($tab_module, ACTION_ADD))
			{
				$imports[]				= array(
						...
						'icon'			=> 'unarchive',
						'label'			=> 'Import PR',
						'onclick'		=> 'modal_quick_add_init(\'temp_prs\')'
				);
				...
				$buttons[]				= array(
						...
						'id'			=> 'add_pr',
						'onclick'		=> 'modal_add_pr_init(\''.$tab_module.'\',\'Add PR\')'
				);
			}
	...
	public function page($encoded_module_code, $encoded_tab_module, $initial=TRUE, $page_num=1)
	{
		try
		{
			...
			$prs						= $this->pr_model->get_purchase_requests_list($where, NULL, $scope_details['having']);
			
			//Get the next batch of internal orders
			$where['limit']['from']		= $page_num * SYS_SETTING_DISPLAY_LIST_NO;
			$next_records 		 		= $this->pr_model->get_purchase_requests_list($where, TRUE, $scope_details['having']);
