...
class Upload_soa extends Task_Controller
{
	...
	public function index()
	{
		try
		{
            ...
            $this->task_view_data['in_charge']       = $this->soa_model->get_cdi_finance_recipient($payment_details['org_code']);
    ...
    private function _validate()
    {
        try
        {
            ...
            ->filter_string('rfp_no')
            ->filter_date('rfp_date')
            ->filter_string('finance_in_charge')
            ->filter();
            ...
            $constraints['rfp_date']	= [
                'data_type' => 'date',
                'name'      => 'RFP Date'
            ];

            $constraints['finance_in_charge']    = [
                'data_type' => 'string',
    ...
    private function _store_payment_info($payment_id, $params) 
    {
        ...
        $activity     = sprintf($this->lang->line('audit_trail_add'), 'SOA');
        $update       = [ 
            'rfp_no'               => $params['rfp_no'],
            'rfp_date'             => $params['rfp_date'],
            'finance_in_charge'    => $params['finance_in_charge']
        ];
