...
class Con_mall_charges extends Task_Controller
{
    ...
    private function _validate()
    {
        try
        {
            ...
            ->filter_string('ok_to_process_payment')
            ->filter_string('confirmation_remarks')
            ->filter();

            $params['task_id'] = decrypt_id($params['etd']);

            $task_document = $this->dm_model->get_document(['pria_task_id' => $params['task_id'], 'document_type_code' => DOC_TYPE_SUPP_DOC]);
            if(EMPTY($task_document))
            {
                $required = [ 'supp_doc' => 'Supporting Document/s' ];

                $constraints['supp_doc'] = [
                    'data_type' => 'string',
            ...
            $required = [ 
                // 'ok_to_process_payment' => 'Ok to Process Payment?',
                'confirmation_remarks'  => 'Confirmation Remarks'
            ];

            // $constraints['ok_to_process_payment']	= [
            //     'data_type' => 'string',
            //     'name'      => 'Ok to Process Payment?'
            // ];

            $constraints['confirmation_remarks']    = [
                'data_type' => 'string',
                'name'      => 'Remarks'
            ];
            ...
            return [
                ...
                'confirmation_remarks'  => $data['confirmation_remarks'],
                'ok_to_process_payment' => (ISSET($params['ok_to_process_payment'])) ? $params['ok_to_process_payment'] : 0 ,
            ];
        }
        catch(PDOException $e)
        {
    ...
    private function _store_payment_info($payment_id, $params) 
    {
        ...
        $activity     = sprintf($this->lang->line('audit_trail_add'), 'Confirm Mall Charges');
        $update       = [ 
            'ok_to_process_payment'    => $params['ok_to_process_payment'],
            'confirmation_remarks'     => $params['confirmation_remarks']
        ];
