...
class Upload_soa_mall_charges extends Task_Controller
{
    ...
    private function _validate()
    {
        try
        {
            ...
            ->filter_string('general_contractor')
            ->filter_string('remarks_for_verification')
            ->filter();

            $params['task_id'] = decrypt_id($params['etd']);

            $task_document = $this->dm_model->get_document(['pria_task_id' => $params['task_id'], 'document_type_code' => DOC_TYPE_CDI_SOA_DOC]);
            if(EMPTY($task_document))
            {
                $required = [ 'cdi_soa_doc' => 'SOA Document' ];

                $constraints['cdi_soa_doc'] = [
                    'data_type' => 'string',
                    'name'      => 'SOA Document'
                ];
            ...
            $required = [ 
                ...
                'remarks_for_verification' => 'Remarks for Verification'
            ];

            $constraints['general_contractor']	= [
                'data_type' => 'string',
                'name'      => 'General Contractor'
            ];

            $constraints['remarks_for_verification']    = [
                'data_type' => 'string',
                'name'      => 'Remarks'
            ];
    ...
    private function _store_payment_info($payment_id, $params) 
    {
        ...
        $activity     = sprintf($this->lang->line('audit_trail_add'), 'SOA for Mall Charges');
        $update       = [ 
            'general_contractor'       => $params['general_contractor'],
            'remarks_for_verification' => $params['remarks_for_verification']
        ];
