...
class Con_avail_rel_payment extends Task_Controller
{
	...
	public function index()
	{
		try
		{
            ...
            $fields         = [
                ...
                'a.remarks_for_verification',

                'a.ok_to_process_payment',
                'a.confirmation_remarks',

                'a.apv_no',
                'a.apv_doc_date',
                'a.transmittal_date_to_bgc',

                'a.check_date',
                'a.date_received_fr_bgc',
                'a.date_received_fr_vendor',
                'a.avail_rel_of_payment_remarks'
            ];
    ...
    private function _validate()
    {
        try
        {
            ...
            ->filter_date('date_received_fr_bgc')
            ->filter_date('date_received_fr_vendor')
            ->filter_string('avail_rel_of_payment_remarks')
            ->filter();
            ...
            $required = [ 
                'check_date'                   => 'Check Date/s',
                'date_received_fr_bgc'         => 'Date Received from BGC',
                'date_received_fr_vendor'      => 'Date Released to Vendor',
                // 'avail_rel_of_payment_remarks' => 'Remarks'
            ];

            $constraints['check_date']	= [
                'data_type' => 'date',
                'name'      => 'Check Date'
            ];

            $constraints['date_received_fr_bgc']    = [
                'data_type' => 'date',
                'name'      => 'Date Received from BGC'
            ];

            $constraints['date_received_fr_vendor']    = [
                'data_type' => 'date',
                'name'      => 'Date Received from Vendor'
            ];

            // $constraints['avail_rel_of_payment_remarks']	= [
            //     'data_type' => 'string',
            //     'name'      => 'Remarks'
            // ];
            ...
            return [
                ...
                'date_received_fr_bgc'         => $data['date_received_fr_bgc'],
                'date_received_fr_vendor'      => $data['date_received_fr_vendor'],
                'avail_rel_of_payment_remarks' => $params['avail_rel_of_payment_remarks']
            ];
        }
        catch(PDOException $e)
        {
    ...
    private function _store_payment_info($payment_id, $params) 
    {
        ...
        $activity     = sprintf($this->lang->line('audit_trail_add'), 'APV Processing');
        $update       = [ 
            'check_date'                   => $params['check_date'],
            'date_received_fr_bgc'         => $params['date_received_fr_bgc'],
            'date_received_fr_vendor'      => $params['date_received_fr_vendor'],
            'avail_rel_of_payment_remarks' => $params['avail_rel_of_payment_remarks']
        ];
