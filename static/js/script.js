    ...
    this.loadView = function(data){
        ...
        if( $('.md-modal.md-show').length > 0 )
        {
            ...
            if( !cloned[ vars.modal_id ] )
            {
                ...
                $('#wrapper').parents('body').find('div.md-overlay:not([style])').last().before(prev_script.clone());
                $('#wrapper').parents('body').find('div.md-overlay:not([style])').last().before(clone);
                cloned[vars.modal_id] = true;
                $('#wrapper').parents('body').find('div.md-overlay:not([style])').last().before(next_script.clone());
            }
            else
            {
    ...
    this.loadModal = function(data){
        ...
        var modalObj = $(target_id).modal({
            ...
            ready: function() {
                ...
                {
                    var api_orig    = $("#" + vars.modal_id).find('div.modal-content').data('jsp');
                    ...
                    $("#" + vars.modal_id).find('div.modal-content').jScrollPane(settings).bind('mousewheel',
                        function(e)
                        {
                            e.preventDefault();
                        }
                    ...
                    api = $("#" + vars.modal_id).find('div.modal-content').data('jsp');  
            ...
            complete: function() {
                ...
                //Added by kebs to clear modal content
                $("#" + vars.modal_id + " .modal-content").find('#content').html('');
